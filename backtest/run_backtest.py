...
from backtest_strategy import BacktestTrader
from data_loader import download_nifty50_data

def run_backtest():
    # 1. Configuration
    DATA_START_DATE = "2022-01-01" # Fetch data from here for MA calculation
    START_DATE = "2023-01-01"      # Start simulation from here
    END_DATE = "2023-12-31"
    INITIAL_BALANCE = 40000        # Updated to 40k
    TRADE_VALUE = 2000             # Updated to 2k per trade
    
    # 2. Load Data
    # Get symbols from strategy (instantiate temp to get list)
    temp_trader = BacktestTrader(None)
    ...
    trader = BacktestTrader(mock_fyers, max_trade_value=TRADE_VALUE)
    
    # 4. Run Simulation
    print(f"Starting backtest from {START_DATE} to {END_DATE}...")
    
    # Use dates from a reliable stock (e.g., RELIANCE.NS) to define trading days
    master_symbol = "NSE:RELIANCE-EQ"
    if master_symbol in data_feed:
        master_df = data_feed[master_symbol]
        # Filter dates within simulation range
        mask = (master_df['date'] >= START_DATE) & (master_df['date'] <= END_DATE)
        dates = master_df.loc[mask, 'date'].tolist()
    else:
        # Fallback if RELIANCE is missing (unlikely)
    ...
    # Prepare JSON data
    # Convert datetime objects in trades to string
    serializable_trades = []
    for t in trades:
        t_copy = t.copy()
        ...
        if '_id' in t_copy:
            del t_copy['_id'] # Remove non-serializable ID
    ...
    import json
    with open('backtest/backtest_metrics.json', 'w') as f:
